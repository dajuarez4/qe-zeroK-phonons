#!/usr/bin/env python3
"""Convert 26 extended 16,000-atom 10 K snapshots to TDEP input."""
...
def main():
    ...
    with (HERE/"infile.positions").open("w") as pos, \
        ...
        for step,rows in read_dump(HERE/"trajectory_10K_extended.lammpstrj"):
            ...
            stat.write(f"{nframes+1} {nframes*1000.0:.8f} {pe+ke:.12e} {pe:.12e} {ke:.12e} {temp:.8f} "+"0.0 "*7+"\n")
            nframes+=1
    (HERE/"infile.meta").write_text(f"{NAT} # N atoms\n{nframes} # N configurations\n1000.0 # sampling interval, fs\n10.0 # temperature, K\n")
