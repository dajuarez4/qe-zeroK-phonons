#!/usr/bin/env python3
"""Analyze the five flat QE EOS outputs, including every relaxation energy."""
...
if len(records) != 5:
    raise SystemExit(f"Expected five completed flat outputs; found {len(records)}")
...
with Path("five_points_all_relaxation_steps.csv").open("w", newline="") as handle:
...
lines = [
    "Five completed AFM alpha-Fe2O3 EOS calculations",
    "",
...
lines.extend(
    [
        "",
        "REQUESTED FIT: every reported relaxation energy (40 observations)",
        ...
        "Repeated volumes are also weighted unequally (7, 8, 7, 9, and 9 energies).",
        "",
        "STANDARD FIT: final relaxed energy at each volume (5 observations)",
        ...
        "Warning: all five volumes are on the compressed side; V0 is extrapolated.",
        "Finish V102-V108 before treating either fit as a physical result.",
    ]
)
summary = "\n".join(lines) + "\n"
Path("five_points_birch_murnaghan_summary.txt").write_text(summary)
print(summary)

energy_reference = final_energy.min()
grid = np.linspace(final_volume.min() * 0.995, max(all_result["V0"], final_result["V0"]) * 1.01, 600)
...
ax_eos.scatter(
    ...
    label="all 40 relaxation energies",
)
ax_eos.scatter(
    final_volume, (final_energy - energy_reference) / 2.0,
    s=48, color="firebrick", zorder=4, label="5 final relaxed energies",
)
ax_eos.plot(
...
ax_eos.set_title(r"Five completed AFM $\alpha$-Fe$_2$O$_3$ EOS points")
...
fig.tight_layout()
fig.savefig("alpha_Fe2O3_five_points_all_steps_bm.png", dpi=230)
fig.savefig("alpha_Fe2O3_five_points_all_steps_bm.pdf")
